...
function Sphere() {
  ...
  useFrame((state) => {
    ...
    state.camera.position.lerp({ x: 50, y: 25, z: zoom ? 50 : -50 } as THREE.Vector3, 0.03)
  ...
  return (
    <mesh ref={ref} receiveShadow castShadow onClick={() => set(!zoom)} onPointerOver={() => setActive(true)} onPointerOut={() => setActive(false)}>
      ...
      <meshStandardMaterial color={active ? 'hotpink' : 'lightblue'} roughness={0} metalness={0.25} />
    </mesh>
  )
}

const Plane = ({ color, ...props }: {color: THREE.Color | string, [key: string]: any}) => (
